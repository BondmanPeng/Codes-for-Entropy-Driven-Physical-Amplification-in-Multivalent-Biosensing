...
public:    
    ...
    void initialize_system(double given_mu_g, double given_rho_r, double given_mu_linker, double given_mu_m, double given_nl, double given_kr,double given_kl,double given_xi_r, double given_xi_l, double given_xim,double given_fcnf,double given_area);
...
void System::initialize_system(double given_mu_g, double given_rho_r, double given_mu_linker, double given_mu_m,double given_nl, double given_kr,double given_kl,double given_xi_r, double given_xi_l, double given_xim,double given_fcnf,double given_area){
    alpha=0;
    L=sqrt(given_area);
    ...
    Ng=0; theta=0; 
    mu_g=given_mu_g; rho_r=given_rho_r; mu_linker=given_mu_linker; mu_m=given_mu_m;
    nl=given_nl;
    kr=given_kr; kl=given_kl; xi_r=given_xi_r; xi_l=given_xi_l; xim=given_xim;
    fcnf=given_fcnf;
...
void System::initialize_interactions(void){
    ...
    for(int i=1;i<max_Nr;++i){
        ...
        Eal=bar_nl*exp(mu_m)*xim;
        Ear=bar_nr*exp(mu_m)*xim;
        ...
        ref_Eal=ref_bar_nl*exp(mu_m)*xim;
        ref_Ear=ref_bar_nr*exp(mu_m)*xim;

        Feff=Feff-nl*log(ref_bar_nl)+ref_bar_nl+ref_summl
                    -nr*log(ref_bar_nr)+ref_bar_nr+ref_summr+ref_Eal+ref_Ear;
        interactions[i]=Feff;
        cout<<"bar_nr: "<<bar_nr<<endl;
        cout<<"bar_nl:"<<bar_nl<<endl;
        cout<<Feff<<endl;
    }
}
...
//         sumkmr=exp(mu_linker)*kr*block_r*pow(block_r+1.,kr-1.);
//         sumkqr=exp(mu_linker-fcnf)*kr*block_r*pow(block_r+1.,kr-1.)*(pow(block_l+1.,kl)-1.);
//         Eal=bar_nl*exp(mu_lm)*xim;
//         Ear=bar_nr*exp(mu_rm)*xim;
...
void System::picard_solve_system(Guest* the_particle){
    ...
    while(error>0.000000001){
        block_l=the_particle->bar_nl*xi_l;
        block_r=the_particle->bar_nr*xi_r;
        sumkml=exp(mu_linker)*kl*xi_l*pow(block_l+1.,kl-1.); // have divided by bar_nl already!
        ...
        func_l=1+(sumkml+sumkql+Eal);
        func_r=1+(sumkmr+sumkqr+Ear);
...
void System::picard_solve_ref_system(Guest* the_particle){
    ...
    while(error>0.000000001){
        ...
        block_l=the_particle->ref_bar_nl*xi_l;
        block_r=the_particle->ref_bar_nr*xi_r;
        sumkml=exp(mu_linker)*kl*xi_l*pow(block_l+1,kl-1); // have been divided bar_nl!
        sumkmr=exp(mu_linker)*kr*xi_r*pow(block_r+1,kr-1);
        Eal=exp(mu_m)*xim;
        Ear=exp(mu_m)*xim;
        ...
        old_bar_nl=the_particle->ref_bar_nl; old_bar_nr=the_particle->ref_bar_nr;
        the_particle->ref_bar_nl=the_particle->ref_bar_nl*0.99+0.01*the_particle->nl/func_l;
        the_particle->ref_bar_nr=the_particle->ref_bar_nr*0.99+0.01*the_particle->nr/func_r;
        error=max(fabs(old_bar_nl-the_particle->ref_bar_nl)/0.01,fabs(old_bar_nr-the_particle->ref_bar_nr)/0.01);
    }
}
...
void System::picard_solve_ref_system_restore(double& ref_bar_nl, double& ref_bar_nr,double nr){
    ...
    while(error>0.000000001){
        ...
        sumkml=exp(mu_linker)*kl*xi_l*pow(block_l+1,kl-1);
        sumkmr=exp(mu_linker)*kr*xi_r*pow(block_r+1,kr-1);
        ref_Eal=exp(mu_m)*xim;
        ref_Ear=exp(mu_m)*xim;

        func_l=1+(sumkml+ref_Eal);
        func_r=1+(sumkmr+ref_Ear);
...
void System::cal_Feff(Guest* the_particle){
    ...
    summr=exp(mu_linker)*(block_r-1);
    sumb=exp(mu_linker-fcnf)*(block_l*block_r-block_l-block_r+1);
    Eal=the_particle->bar_nl*exp(mu_m)*xim;
    Ear=the_particle->bar_nr*exp(mu_m)*xim;
    ...
    ref_summl=exp(mu_linker)*(ref_block_l-1);
    ref_summr=exp(mu_linker)*(ref_block_r-1);
    ref_Eal=the_particle->ref_bar_nl*exp(mu_m)*xim;
    ref_Ear=the_particle->ref_bar_nr*exp(mu_m)*xim;

    the_particle->Feff=the_particle->Feff-the_particle->nl*log(the_particle->ref_bar_nl)+the_particle->ref_bar_nl+ref_summl
                -the_particle->nr*log(the_particle->ref_bar_nr)+the_particle->ref_bar_nr+ref_summr+ref_Eal+ref_Ear;
